...
function App() {
  ...
  const { initialize, language, changeLanguage, activeSectionId, activeOrganizationId, updateCtx, openDetail } = useViewStore();
  const { data, isLoading: isInventoryLoading, error } = useInventoryQuery();
  const { setUser, user: authUser, isLoading: isAuthLoading, setLoading } = useAuthStore();

  const [isAddModalOpen, setIsAddModalOpen] = useState(false);
  // View state for organization workflow
  const [viewState, setViewState] = useState<'main' | 'org-select' | 'org-create'>('main');
  ...
  if (viewState === 'org-select') {
    return <OrganizationSelect onCreateClick={() => setViewState('org-create')} />;
  }
    ...
    <MainLayout>
      ...
      {!activeSectionId ? (
        <div className="text-center p-10 bg-white rounded-lg shadow mt-10">
          ...
          <p className="text-sm text-gray-400">Please select or create settings from the sidebar.</p>
        </div>
      ) : (
        <>
          {isInventoryLoading && <p className="text-center py-10">Loading...</p>}
          {error && <p className="text-red-500 text-center py-10">Error loading inventory</p>}
